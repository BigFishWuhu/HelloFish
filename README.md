...
MXU 负责设备连接、任务配置、运行状态和日志展示。“语音厅贡献榜扫描”默认加入任务列表。Agent 启动后，可在任务说明中点击“查看贡献记录”，或直接访问 [http://127.0.0.1:8765/](http://127.0.0.1:8765/)；只有点击链接时才会打开浏览器，无需再运行额外任务。页面仅限本机访问，支持按日期、近 N 天、财富等级、性别、挚友数量、厅 ID、用户 ID 和用户名筛选，点击用户名或用户 ID 区域均会复制用户 ID，可持久化隐藏指定厅 ID 或用户 ID，并可在贡献值与金额之间切换（10 贡献值 = 1 元）。CSV 导出会沿用当前筛选条件，可按需勾选导出列。
