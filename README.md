...
逐项读取用户名、公开 ID、性别、IP 属地、挚友数量和排名。任务设置中可指定每个厅扫描贡献榜前多少名，默认 100 名。默认允许每次任务重复扫描同一个厅；可按需开启“跳过今天已扫描的厅”，该选项默认关闭，且绝不会跨天跳过。性别按详情页 ID 复制按钮同一行的 ♂/♀ 图标识别。同一厅、同一用户在同一天重复扫描时覆盖更新，跨天记录继续保留。
