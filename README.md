...
MXU 负责设备连接、任务配置、运行状态和日志展示。“语音厅贡献榜扫描”默认加入任务列表。贡献记录服务启动时会由操作系统分配随机可用的本机端口，实际访问地址会写入启动日志（Agent 日志也会打印该地址）；停止任务不会关闭页面，后台服务会在退出 MXU 时停止。页面仅限本机访问，支持按精确到分钟的自定义时间范围、近 N 天、财富等级、性别、挚友数量、厅 ID、用户 ID 和用户名筛选，点击用户名或用户 ID 区域均会复制用户 ID，可持久化隐藏指定厅 ID 或用户 ID，并可在贡献值与金额之间切换（10 贡献值 = 1 元）。CSV 和静态 HTML 导出都会沿用当前筛选条件，CSV 可按需勾选导出列。
...
在通用 UI 中运行“语音厅贡献榜扫描”即可，任务会自动打开双鱼部落 APK（包名 `com.sybl.voiceroom`），并在首次读取厅列表前返回厅列表页、连续下拉刷新两次以更新厅列表排序；不需要预先手动停留在厅列表页。
任务会依次进入未扫描的厅，打开右上角皇冠入口，切换到“房间贡献榜”，
直接逐页扫描贡献榜，从无障碍结构读取排名、公开 ID 和“距前一名”，再逐项读取用户名、性别、IP 属地和挚友数量，不再额外预扫描或估算全榜贡献值。任务设置中可指定每个厅扫描贡献榜前多少名，默认 100 名；可设置跳过的厅 ID 和厅名称，名称会忽略大小写、空格、标点并进行模糊匹配（例如“红馆”可匹配“1010红人馆”）；还可用复选框选择需要记录的男、女、未知性别，默认全部勾选。识别到未勾选性别的用户会立即返回贡献榜。默认允许每次任务重复扫描同一个厅；可按需开启“跳过今天已扫描的厅”，该选项默认关闭，且绝不会跨天跳过。性别按详情页 ID 复制按钮同一行的 ♂/♀ 图标识别。同一厅、同一用户在同一天重复扫描时覆盖更新，跨天记录继续保留。
...
- `data/voice_hall.sqlite3`：新的唯一业务数据源。
  - `contributions`：用户贡献榜记录，包含距前一名；推测贡献值字段保留用于兼容已有数据。同厅、同用户、同一天由唯一键覆盖更新。
...
`VoiceHallScanStart.custom_action_param` 中调整扫描上限、跳过厅 ID、跳过厅名称、记录性别和 SQLite 输出路径。CSV 导出的金额列按金额大小显示为 `元` 或 `万元`，例如 `9999元`、`1万元`。
