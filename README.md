...
MXU 负责设备连接、任务配置、运行状态和日志展示。“语音厅贡献榜扫描”默认加入任务列表。每次启动 MXU 时会清理上次运行留下的日志。贡献记录服务启动时会由操作系统分配随机可用的本机端口，实际访问地址会写入启动日志（Agent 日志也会打印该地址）；停止任务不会关闭页面，后台服务会在退出 MXU 时停止。贡献数据仅保留最近 7 个自然日。页面仅限本机访问，支持按精确到分钟的自定义时间范围、近 N 天、财富等级、性别、挚友数量、厅 ID、用户 ID 和用户名筛选，点击用户名或用户 ID 区域均会复制用户 ID，可持久化隐藏指定厅 ID 或用户 ID，并可在贡献值与金额之间切换（10 贡献值 = 1 元）。CSV 和静态 HTML 导出都会沿用当前筛选条件，CSV 可按需勾选导出列。
